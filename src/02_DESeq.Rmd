...
rowData(x) <- left_join(
	...
	as.data.frame(gtf) |> select(feature_id=gene_id,contig=seqnames,gene_biotype,old_locus_tag),
...
x$cond <- local({set.seed(321);sample(c("condA","condB"),size = ncol(x),replace = TRUE)})
...
all_deseq_results <- list(
	condAvsB = as.data.frame(results(x))
) |>
